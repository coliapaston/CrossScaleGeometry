...
def compute_pearson_stats(x: Sequence[float], y: Sequence[float]) -> PearsonResult:
    x = np.asarray(x, dtype=float)
    y = np.asarray(y, dtype=float)
    if x.shape != y.shape:
        raise ValueError(f"Pearson inputs must have the same shape, got {x.shape} vs {y.shape}")
    if x.size < 2:
        raise ValueError(f"Pearson requires at least 2 observations, got n={x.size}")

    rho, pvalue = pearsonr(x, y)
    return PearsonResult(r=float(rho), pvalue=float(pvalue), n_obs=int(x.size))
...
def compute_grouped_permutation_pvalue(
    ...
    *,
    ...
    observed_r = float(pearsonr(baseline_vec, seed_vec).statistic)
    ...
    for _ in range(int(n_permutations)):
        ...
        perm_r = float(pearsonr(baseline_vec, perm_seed_vec).statistic)
...
def compare_seed_to_baseline_for_metric(
    ...
    *,
    ...
    mean_result = compute_pearson_stats(*build_mean_vectors(aligned))
    std_result = compute_pearson_stats(*build_std_vectors(aligned))
    ...
    return {
        ...
        "mean_r": mean_result.r,
        "mean_p": mean_result.pvalue,
        "std_r": std_result.r,
        "std_p": std_result.pvalue,
        "mean_std_joint_r": mean_std_perm.r,
...
def compare_seed_to_baseline_joint_mean(
    ...
    *,
    ...
    rho = float(pearsonr(baseline_vec, seed_vec).statistic)
...
def run_pearson_over_random_seeds(
    *,
    out_root: str | Path,
    model_name: str,
    space_name: str,
...
def save_pearson_summary_df(df: pd.DataFrame, outpath: str | Path) -> Path:
...
def make_main_pearson_latex_table(
    summary_df: pd.DataFrame,
    *,
    precision: int = 3,
    model_order: Optional[Sequence[str]] = None,
    caption: str = "Main Pearson stability results across random PCA seeds.",
    label: str = "tab:pearson_main",
) -> str:
    required_cols = ["model_name", "seed", "morph_mean_r", "script_mean_r", "joint_mean_r"]
    ...
    grouped = (
        ...
        .agg(
            ...
            morph_mean_r_mean=("morph_mean_r", "mean"),
            morph_mean_r_std=("morph_mean_r", "std"),
            script_mean_r_mean=("script_mean_r", "mean"),
            script_mean_r_std=("script_mean_r", "std"),
            joint_mean_r_mean=("joint_mean_r", "mean"),
            joint_mean_r_std=("joint_mean_r", "std"),
        )
        .reset_index()
    )
    ...
    table_df = pd.DataFrame(
        {
            ...
            "Morph Mean $r$": [
                ...
                for m, s in zip(grouped["morph_mean_r_mean"], grouped["morph_mean_r_std"])
            ],
            "Script Mean $r$": [
                _format_mean_std(m, s, precision=precision)
                for m, s in zip(grouped["script_mean_r_mean"], grouped["script_mean_r_std"])
            ],
            "Joint Mean $r$": [
                _format_mean_std(m, s, precision=precision)
                for m, s in zip(grouped["joint_mean_r_mean"], grouped["joint_mean_r_std"])
            ],
        }
    )
...
def make_appendix_pearson_latex_table(
    summary_df: pd.DataFrame,
    *,
    precision: int = 3,
    model_order: Optional[Sequence[str]] = None,
    caption: str = "Full seed-level Pearson results.",
    label: str = "tab:pearson_appendix",
    longtable: bool = True,
) -> str:
    required_cols = [
        "model_name",
        ...
        "script_std_r",
        "script_std_p",
        "morph_mean_std_joint_r",
        "morph_mean_std_joint_perm_p",
        "script_mean_std_joint_r",
    ...
    table_df = table_df.rename(
        columns={
            "model_name": "Model",
            "seed": "Seed",
            "morph_mean_r": "Morph Mean $r$",
            "morph_mean_p": "Morph Mean $p$",
            "script_mean_r": "Script Mean $r$",
            "script_mean_p": "Script Mean $p$",
            "joint_mean_r": "Joint Mean $r$",
            "joint_mean_perm_p": "Joint Mean Perm. $p$",
            "morph_std_r": "Morph Std $r$",
            "morph_std_p": "Morph Std $p$",
            "script_std_r": "Script Std $r$",
            "script_std_p": "Script Std $p$",
            "morph_mean_std_joint_r": "Morph Mean+Std $r$",
            "morph_mean_std_joint_perm_p": "Morph Mean+Std Perm. $p$",
            "script_mean_std_joint_r": "Script Mean+Std $r$",
...
def make_permutation_vs_full_latex_table(
    ...
    *,
    precision: int = 3,
    model_order: Optional[Sequence[str]] = None,
    caption: str = "Pearson comparison between the original full-PCA results and the permutation control.",
    label: str = "tab:perm_vs_full_pearson",
) -> str:
    required_cols = [
        "baseline_model_name",
        "morph_mean_r",
